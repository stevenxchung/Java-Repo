...
/**
 * Steven Chung, Udacity interactive Tic-Tac-Toe project in Java
...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        String result = "None";
        Character[] varXO = {'x', 'o'};
        String[] dispo = {"Player X wins the round!", "Player O wins the round!"};

        // Iterate over the rows of the matrix varXO which contains 'x' and 'o' characters (2x1 matrix)
        for (int i = 0; i < varXO.length; i++) {

            // First check if the grid diagonal at (0, 0), (1, 1), and (2, 2) contain three x's or o's
            if (grid[0][0] == varXO[i] && grid[1][1] == varXO[i] && grid[2][2] == varXO[i])
                return dispo[i];
            // Second check if the grid diagonal at (2, 0), (1, 1), and (0, 2) contain three x's or o's
            if (grid[2][0] == varXO[i] && grid[1][1] == varXO[i] && grid[0][2] == varXO[i])
                return dispo[i];

            // Iterate over the rows of the grid for the length of the grid (3x3 matrix)
            for (int k = 0; k < grid.length; k++) {

                // Check each row of the grid to see if there are either three x's or o's in a row
                if (grid[0][k] == varXO[i] && grid[1][k] == varXO[i] && grid[2][k] == varXO[i])
                    return dispo[i];

                // Check each row of the grid to see if there are either three x's or o's in a column
                if (grid[k][0] == varXO[i] && grid[k][1] == varXO[i] && grid[k][2] == varXO[i])
                    return dispo[i];

            }
        }

        // One method to check for a tie is if there is no instance three 'x' or 'o' is aligned
        int count = 0;
        // Step through the rows of the grid
        for (int r = 0; r < grid.length; r++) {
            // Step through columns of the grid
        ...
        // Both players have tied the round if all spaces have been filled and there are no consecutive 'x' or 'o'
        if (count == 9)
            return "Both players have tied this round!";
